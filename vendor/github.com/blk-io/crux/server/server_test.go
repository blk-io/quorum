...
func TestPushGrpc(t *testing.T) {
	...
	port := 9015
	url := fmt.Sprintf("http://%s:%d/push", "localhost", port)

	//InitgRPCServer(t, true, port)
	ipcPath, err := ioutil.TempDir("", "TestInitIpc")
	_, err = Init(&MockEnclave{}, port, ipcPath, true, false, "", "")
	...
	//resp, err := c.Post(url,"application/octet-stream", r)
